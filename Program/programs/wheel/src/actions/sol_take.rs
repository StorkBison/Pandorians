...
#[derive(Accounts)]
#[instruction(params: SolTakeParams)]
pub struct SolTake<'info> {
    #[account(mut,
...
pub struct SolTakeParams {
    pub force: [u8; 32],
    pub wheel_id: u16,
}

impl SolTake<'_> {
    pub fn validate(&self, _ctx: &Context<Self>, _params: &SolTakeParams) -> Result<()> {
        Ok(())
    }

    pub fn actuate(ctx: &Context<Self>, _params: &SolTakeParams) -> Result<()> {
        ...
        let vrf_status: std::cell::Ref<'_, VRFStatus> = ctx.accounts.vrf_status.load()?;
            ...
            if let Some(prizex) = wheel_state.prizes.get(prize_index as usize) {
                invoke_signed(
                    &system_instruction::withdraw_nonce_account(
                        ...
                        prizex.amount * vrf_status.reward,
                    ),
                    &[
                ...
                wl_limit += (u32::MAX as f32 * 0.005 * wheel_state.prizes[prize_index as usize].probability) as u32;
                if result <= wl_limit {
                    transfer(
